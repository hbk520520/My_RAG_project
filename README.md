...
（1）我们要使用三个模型，第一个是Meta-planner，负责为原问题生成一个抽象的推理骨架；第二个是Control-layer，或者是叫replaner，负责当查询不到东西的时候修改流程；原本其负责跟踪子问题的解决程度，即这个问题收集到的资料可以解决了吗，对于检查不到的地方是不是要进行重写，后面想了想需要进行解耦，因为参数量不够，而且后面我们会用Reasoner的结果去反推有没有检索正确；第三个是Extractor，从抽取到的文档精简事实。
